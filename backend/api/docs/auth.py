...
_success_registration_example = Tokens(
    ...
    user=User(name="Имя пользователя", surname="Фамилия пользователя", login="user", id=1),
).dict()
    ...
    responses=[
        StatusCodeDocResponseExample(
            ...
            response_example=DocResponseExample(
                description="Успешная регистрация", example=_success_registration_example
            ),
        ),
        StatusCodeDocResponseExample(
...
login_responses = DocResponses(
    responses=[
        ...
        StatusCodeDocResponseExample(
            ...
            response_example=DocResponseExample(description="Неверный пароль", example=_wrong_password_login_example),
        ),
        StatusCodeDocResponseExample(
            status_code=status.HTTP_404_NOT_FOUND,
            response_example=DocResponseExample(
                description="Пользователь не найден", example=_user_not_exist_login_example
            ),
        ),
    ]
).to_openapi()
...
refresh_responses = DocResponses(
    responses=[
        StatusCodeDocResponseExample(
            status_code=status.HTTP_200_OK,
            response_example=DocResponseExample(
                description="Успешное обновление токенов", example=_success_refresh_example
            ),
        ),
        StatusCodeDocResponseExample(
            status_code=status.HTTP_401_UNAUTHORIZED,
            response_example=DocResponseExample(
                description="Не удалось обновить токены", example=_not_success_refresh_example
            ),
        ),
    ]
).to_openapi()
...
logout_responses = DocResponses(
    responses=[
        StatusCodeDocResponseExample(
            status_code=status.HTTP_200_OK,
            response_example=DocResponseExample(description="Успешный выход", example=_success_logout_example),
        ),
        StatusCodeDocResponseExample(
            status_code=status.HTTP_401_UNAUTHORIZED,
            response_example=DocResponseExample(description="Не корректный refresh token", example=_bad_logout_example),
        ),
    ]
).to_openapi()
