...
_not_chat_member_status_code_response_example = StatusCodeDocResponseExample(
    ...
    response_example=DocResponseExample(
        description="Запрос участников чата от не участника чата", example=_user_not_chat_member_example
    ),
)
...
get_chat_members_responses = DocResponses.create_instance_with_not_auth_response(
    responses=[
        StatusCodeDocResponseExample(
            ...
            response_example=DocResponseExample(
                description="Успешное получение списка участников чата", example=_success_get_chat_members_example
            ),
        ),
        StatusCodeDocResponseExample(
            status_code=status.HTTP_403_FORBIDDEN,
        ...
        StatusCodeDocResponseExample(
            ...
            response_example=DocResponseExample(
                description="Запрос участников не существующего чата", example=_chat_not_exist_example
            ),
        ),
    ]
).to_openapi()
...
add_chat_member_responses = DocResponses.create_instance_with_not_auth_response(
    responses=[
        StatusCodeDocResponseExample(
            ...
            response_example=DocResponseExample(
                description="Успешное добавление участника к чату", example=_success_add_chat_member_example
            ),
        ),
        StatusCodeDocResponseExample(
            status_code=status.HTTP_403_FORBIDDEN,
            response_example=DocResponseExample(
                description="Добавление в чат от не участника чата", example=_user_not_chat_member_example
            ),
        ),
        StatusCodeDocResponseExample(
            status_code=status.HTTP_404_NOT_FOUND,
            response_example=DocResponseExample(
                description="Попытка добавить не существующего пользователя в чат", example=_not_exist_user_example
            ),
        ),
    ]
).to_openapi()
...
delete_chat_member_responses = DocResponses.create_instance_with_not_auth_response(
    responses=[
        StatusCodeDocResponseExample(
            status_code=status.HTTP_200_OK,
            response_example=DocResponseExample(
                description="Успешное удаление участника из чата", example=_success_delete_chat_member_example
            ),
        ),
        StatusCodeDocResponseExample(
            status_code=status.HTTP_403_FORBIDDEN,
            response_example=DocResponseExample(
                description="Только создатель может удалять из чата", example=_user_is_not_chat_creator_example
            ),
        ),
        StatusCodeDocResponseExample(
            status_code=status.HTTP_404_NOT_FOUND,
            response_example=DocResponseExample(
                description="Не возможно удалить не участника чата", example=_login_is_not_chat_member_example
            ),
        ),
    ]
).to_openapi()
