...
_success_get_user_info_example = User(
    id=1, login="user", name="Имя пользователя", surname="Фамилия пользователя"
).dict()
    ...
    responses=[
        StatusCodeDocResponseExample(
            ...
            response_example=DocResponseExample(
                description="Успешное изменение данных пользователя", example=_success_change_user_data_example
            ),
        )
    ]
).to_openapi()
...
get_user_info_responses = DocResponses.create_instance_with_not_auth_response(
    responses=[
        StatusCodeDocResponseExample(
            status_code=status.HTTP_200_OK,
            response_example=DocResponseExample(
                description="Успешное получение информации о пользователе", example=_success_get_user_info_example
            ),
        ),
        StatusCodeDocResponseExample(
            status_code=status.HTTP_404_NOT_FOUND,
