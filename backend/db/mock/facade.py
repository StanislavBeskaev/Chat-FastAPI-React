...
class MockDBFacade(DBFacadeInterface):
    ...
    def get_all_profiles(self) -> list[models.Profile]:
        ...
        return self.users_dao.get_all_profiles()
    ...
    def get_all_refresh_tokens(self) -> list[models.RefreshToken]:
        ...
        return self.tokens_dao.get_all_refresh_tokens()
    ...
    def find_refresh_token_by_user(self, user_id: int, user_agent: str) -> tables.RefreshToken | None:
        ...
        return self.tokens_dao.find_refresh_token_by_user(user_id=user_id, user_agent=user_agent)
    ...
    def find_refresh_token_by_token(self, token: str, user_agent: str) -> tables.RefreshToken | None:
        ...
        return self.tokens_dao.find_refresh_token_by_token(token=token, user_agent=user_agent)

    def create_refresh_token(self, user_id: int, refresh_token: str, user_agent: str) -> tables.RefreshToken:
        """Создание refresh токена"""
        return self.tokens_dao.create_refresh_token(user_id=user_id, refresh_token=refresh_token, user_agent=user_agent)

    def update_refresh_token(self, token: tables.RefreshToken, new_refresh_token: str) -> tables.RefreshToken:
        """Обновление refresh токена"""
        return self.tokens_dao.update_refresh_token(token=token, new_refresh_token=new_refresh_token)

    def delete_refresh_token(self, token: str, user_agent: str) -> None:
        """Удаление refresh токена"""
        self.tokens_dao.delete_refresh_token(token=token, user_agent=user_agent)

    def delete_all_refresh_tokens(self) -> None:
        """Удаление всех refresh токенов. Нужен для тестов"""
        self.tokens_dao.delete_all_refresh_tokens()

    def get_all_chats(self) -> list[models.Chat]:
        """Получение всех записей из таблицы чатов"""
        return self.chats_dao.get_all_chats()

    def get_chat_by_id(self, chat_id: str) -> models.Chat:
        """Получение чата по id"""
        return self.chats_dao.get_chat_by_id(chat_id=chat_id)

    def create_chat(self, chat_name: str, creator_id: int) -> models.Chat:
        """Создание нового чата"""
        return self.chats_dao.create_chat(chat_name=chat_name, creator_id=creator_id)
    ...
    def get_chat_members(self, chat_id: str) -> list[models.User]:
        ...
        return self.chat_members_dao.get_chat_members(chat_id=chat_id, users_dao=self.users_dao)

    def find_chat_member(self, user_id: int, chat_id: str) -> tables.ChatMember | None:
        """Поиск участника чата"""
        return self.chat_members_dao.find_chat_member(user_id=user_id, chat_id=chat_id)

    def add_chat_member(self, user_id: int, chat_id: str) -> None:
        """Добавление участника к чату"""
        self.chat_members_dao.add_chat_member(user_id=user_id, chat_id=chat_id)

    def delete_chat_member(self, chat_member: tables.ChatMember) -> None:
        """Удаление участника из чата"""
        self.chat_members_dao.delete_chat_member(chat_member=chat_member)

    def get_all_messages(self) -> list[models.MessageFull]:
        """Получение всех записей из таблицы сообщений"""
        return self.messages_dao.get_all_messages()
    ...
    def get_all_read_status_messages(self) -> list[models.MessageReadStatus]:
        ...
        return self.messages_dao.get_all_read_status_messages()

    def create_text_message(self, text: str, user_id: int, chat_id: str) -> models.Message:
        """Создание текстового сообщения в базе"""
        return self.messages_dao.create_text_message(text=text, user_id=user_id, chat_id=chat_id)
    ...
    def create_unread_messages(self, message: models.Message, chat_members: list[models.User]) -> None:
        ...
        self.messages_dao.create_unread_messages(message=message, chat_members=chat_members)
    ...
    def create_info_message(self, text: str, user_id: int, chat_id: str) -> models.Message:
        ...
        return self.messages_dao.create_info_message(text=text, user_id=user_id, chat_id=chat_id)
    ...
    def get_user_chat_messages(self, user_id: int, chat_id: str) -> list[models.ChatData]:
        ...
        return self.messages_dao.get_user_chat_messages(
            user_id=user_id,
            chat_id=chat_id,
    ...
    def get_message_by_id(self, message_id: str) -> tables.Message:
        ...
        return self.messages_dao.get_message_by_id(message_id=message_id)

    def change_message_text(self, message_id: str, new_text: str) -> tables.Message:
        """Изменение текста сообщения"""
        return self.messages_dao.change_message_text(message_id=message_id, new_text=new_text)
    ...
    def get_all_contacts(self) -> list[models.ContactFull]:
        ...
        return self.contacts_dao.get_all_contacts()

    def get_user_contacts(self, user_id: int) -> list[models.Contact]:
        """Получение контактов пользователя"""
        return self.contacts_dao.get_user_contacts(user_id=user_id, users_dao=self.users_dao)

    def find_contact(self, owner_user_id: int, contact_user_id: int) -> tables.Contact | None:
        """Нахождение контакта пользователя"""
        return self.contacts_dao.find_contact(owner_user_id=owner_user_id, contact_user_id=contact_user_id)
    ...
    def delete_contact(self, contact: tables.Contact) -> None:
        """Удаление контакта"""
        self.contacts_dao.delete_contact(contact=contact)
